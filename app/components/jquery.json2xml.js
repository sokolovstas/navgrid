...
(function(jQuery) {
	 ...
	 * @return XML string 
	 */
	jQuery.json2xml = function(json, options) {
		settings = {};
		settings = jQuery.extend(true, settings, defaultSettings, options || { });
	...
	var convertToXml = function(json, tagName, parentPath, depth) {
		...
		for (var key in json) {
			if (json.hasOwnProperty(key)) {
				...
				if (jQuery.inArray(propertyPath, settings.ignore) == -1) {
					// array, create new child element
					if (jQuery.isArray(json[key])) {
					...
					// primitive value property as attribute
						...
						if (jQuery.inArray(propertyPath, settings.nodes) != -1) {
